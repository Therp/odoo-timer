...
(function() {
    ...
    window.addEventListener("load", function() {
        ...
        actionFooter.className = "actionfooter";
        el.appendChild(body);
        el.appendChild(actionFooter);
    ...
    function setAlertBodyContent(content) {
        var body = alertBox.childNodes[0];
        while (body.firstChild) {
            body.removeChild(body.firstChild);
        }

        // Callers may pass a DOM node for structured content such as the
        // timesheet-description prompt. All ordinary messages are rendered as
        // text, so remote/server values cannot become executable markup.
