...
function createPopupAppTemplate(app, bdom, helpers) {
  ...
  return function template(ctx, node, key = '') {
    ...
    const showAllIssuesHandler = [(ev) => { ctx.updateShowAllPreference(ev.target.checked); }];
...
class PopupApp extends Component {
  ...
  async refreshAll() {
    try {
      ...
      alert(err.message || 'Failed to refresh items.');
    }
  }
  ...
  async stopTimer(issue) {
    try {
      ...
      if (this.state.dataSource === DATA_SOURCE_ISSUE) {
        ...
        alert(`Time for ${this.itemLabelSingular} #${issue.id} was successfully recorded in Odoo timesheets.`);
      } else {
      ...
      console.error(err);
      alert(err.message || `Could not stop the ${this.itemLabelSingular} timer and create a timesheet.`);
    }
  }
  ...
  async resetTimer() {
    ...
    const confirmed = confirm(`Discard the running ${this.itemLabelSingular} timer without saving to Odoo?`);
    if (!confirmed) {
      return;
    }
